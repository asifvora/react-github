...
export default function StarItem({ repo }) {
    ...
    return (
        <li className="border-bottom" style={{ paddingBottom: '50px', paddingTop: '5px' }}>
            <h3><a href={repo.html_url} style={{ color: '#0366d6' }} target="_blank" title={repo.name}>{repo.name}</a></h3>
            <p className="text-gray">{repo.description}</p>
            <div className="pull-left">
                <strong>{repo.language}</strong>
                <strong><i className="fa fa-star"></i> {repo.stargazers_count}</strong>
                <strong><i className="fa fa-code-fork" /> {repo.forks_count}</strong>
